...
(run-matcher (match:compile-pattern '(?:choice a (? x ,symbol?))) 'b print-all-matches)

(run-matcher (match:compile-pattern '(?:choice a b (? x) c))
             'z
             match:bindings)
...
;all outputs were #f so there is bug somewhere
