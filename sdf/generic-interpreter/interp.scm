...
(define-generic-procedure-handler g:eval
  (match-args string? environment?)
  (lambda (expression environment)
    (let lookup ((str expression))
      (display str) (newline)
      (display expression) (newline)
      (display "first break") (newline)
      (display environment) (newline)
      (if (string-null? str) ""
	  (let ((r (string-search-forward "kennedy" str)))
            (display r) (newline)
	    (if (false? r)
		str
		(let* ((s (substring-search-forward " " str r
			      ...
			      s))
                       (display r) (newline)
                       (display t) (newline)
		       (sub (substring str r t))
                       (display "break") (newline)
                       (display sub) (newline)
		       (val (if (false? (string->number sub))
				(let ((out (g:eval (string->symbol sub) environment)))
                                  (if (string? out) out
				      (write-to-string out)))
				sub)))
		  ;(bkpt 'ah 'ha)
...
(define-generic-procedure-handler g:apply
  (match-args vector? operands? environment?)
  (lambda (procedure operands calling-environment)
    (list->vector
     (map (lambda (proc) (g:apply proc operands calling-environment))
          (vector->list procedure)))))
;;;5.1 extension

(define-generic-procedure-handler g:apply
  (match-args variable? operands? environment?)
  (lambda (procedure operands calling-environment)
