...
(define medical-center:recovery-rate
  (make-property 'recovery-rate
                 'predicate bias?))
  
(define medical-center?
  (make-type 'medical-center (list medical-center:recovery-rate)))
(set-predicate<=! medical-center? place?)

(define make-medical-center
  (type-instantiator medical-center?))

(define get-recovery-rate
  (property-getter medical-center:recovery-rate medical-center?))

(define (rest-and-recover! medical-center)
  (let ((patients (people-in-place medical-center)))
    (for-each (add-rest-health!) (patients))))

(define-clock-handler medical-center? rest-and-recover!)

;;; Mobile things
...
(define (add-rest-health! person)
  (if (< (get-health person) 30)
      (set-health! person (+ (get-health person) 1))))

(define-clock-handler person? add-rest-health!)

;;; Bags
        ...
        (if (n:null? people)
            ...
            (let ((victim (random-choice people)))
              (narrate! (list troll "takes a bite out of" victim)
                        troll)
              (suffer! (random-number 30) victim))))))


(define-clock-handler troll? eat-people!)

;;; Sword
;;; If a person possesses a sword, user can direct person to attack a particular troll with it.

(define sword:damage-inflicted
  (make-property 'damage-inflicted
                 'predicate bias?))

(define sword?
  (make-type 'sword (list sword:damage-inflicted)))
(set-predicate<=! sword? mobile-thing?)

(define make-sword
  (type-instantiator sword?))

(define get-damage-inflicted
  (property-getter sword:damage-inflicted sword?))

(define (attack-person! sword person1 person2)
  (announce!
   '(person1 "attacks" person2 "with a sword!"))
  (let ((things-in-bag (get-things person1)))
    (if (memq sword things-in-bag)
      (set-health! person2 (- (get-health person) 10)))))

;;; Invisibility Cloak

(define invisibility-cloak:harm
  (make-property 'harm
                 'predicate bias?))
    
(define invisibility-cloak?
  (make-type 'invisibility-cloak (list invisibility-cloak:harm)))
...
(define get-harm
  (property-getter invisibility-cloak:harm invisibility-cloak?))

(define (cause-harm! invisibility-cloak person)
  (let ((things-in-bag (get-things person)))
    (if (memq invisibility-cloak things-in-bag)
      (set-health! person (- (get-health person) 1)))))

(define (make-invisible! invisibility-cloak person)
  (set-location! person null?))

(define-clock-handler invisibility-cloak? cause-harm!)

(define-clock-handler invisibility-cloak? make-invisible!)

;;; Avatars
...
(define-generic-procedure-handler set-up!
  ...
  (lambda (super avatar)
    (super avatar)
    (register-with-clock! avatar (get-clock))))

(define-generic-procedure-handler tear-down!
  (match-args avatar?)
  (lambda (super avatar)
    (unregister-with-clock! avatar (get-clock))
    (super avatar)))
